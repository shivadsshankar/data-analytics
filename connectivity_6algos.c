...
{	
	...
	{
		degree[i] = 0;
		label1[i] = label2[i] = label3[i] = label4[i] = label5[i]= label6[i] = 0;
		for( j = 0; j < MAX_VAL; j ++ )
		{
			edge[i][j] = 0;
		}
	...
	i = 1;	
	while( fscanf( ifp, "%d %d %d %d %d %d", &a1, &a2, &a3, &a4, &a5, &a6 ) != EOF ) //reads the file line by line and stores the values into variables.
	{		
		label1[i] = a1;
		label2[i] = a2;
		label3[i] = a3;
